...
def transcribe_large_audio(path):
    ...
    for i, audio_chunk in enumerate(chunks, start=1):
        ...
        with sr.AudioFile(chunk_filename) as source:
            ...
            # Convert to text
            try:
                print(f"zapisano plik {chunk_filename} o długości {round(audio_chunk.duration_seconds,2)}s. Rozpoznaję tekst...")
                text = r.recognize_google(audio_listened, language="pl-PL")
            except sr.UnknownValueError as e:
                print(f"zapisano plik {chunk_filename} o długości {round(audio_chunk.duration_seconds,2)}s. Error:", str(e))
            else:
    ...
    print(f"zapisuję plik: {org_filename}.txt")        
